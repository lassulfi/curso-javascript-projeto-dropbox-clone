class DropBoxController {
    ...
    removeFolderTask(reference, name) {
        return new Promise((resolve, reject) => {
            let folderReference = this.getFirebaseReference(reference + '/' + name);
            folderReference.on('value', snapshot => {
                folderReference.off('value');
                snapshot.forEach(item => {
                    let data = item.val();
                    data.key = item.key;
                    if(data.type === 'folder'){
                        this.removeFolderTask(reference + '/' + name, data.name).then(() => {
                            resolve({
                                fields:{
                                    key: data.key
                                }
                        ...
                        this.removeFile(reference + '/' + name, data.name).then(() => {
                            resolve({
                                fields:{
                                    key: data.key
                                }
    ...
    removeTask() {
        ...
        this.getSelectedElements().forEach(li => {
    ...
    removeFile(reference, name){
        let fileRef = firebase.storage().ref(reference).child(name);
    ...
    initEvents() {
        this.btnNewFolderEl.addEventListener('click', event => {
            ...
            if(name){
                this.getFirebaseReference().push().set({
                    name,
                    type: 'folder',
            ...
            this.removeTask().then(responses => {
                responses.forEach(response => {
                    if(response.fields.key){
                        this.getFirebaseReference().child(response.fields.key).remove();
                    }
                });
            }).catch(err => {
                console.error(err);
        ...
        this.btnRenameEl.addEventListener('click', event => {
            let li = this.getSelectedElements()[0];
            let file = JSON.parse(li.dataset.file);
            let name = prompt("Renomear aquivo: ", file.name);
            if(name){
                file.name = name;
                this.getFirebaseReference().child(li.dataset.key).set(file);
            }
        });

        this.listFilesEl.addEventListener('selectionchange', e => {
            switch(this.getSelectedElements().length){
                case 0://No file selected
                    this.btnRenameEl.style.display = 'none';
                    this.btnDeleteEl.style.display = 'none';
                    break;
                case 1://one file selected
                    this.btnRenameEl.style.display = 'block';
                    this.btnDeleteEl.style.display = 'block';
                    break;
                default://more than one file selected
                    this.btnRenameEl.style.display = 'none';
                    this.btnDeleteEl.style.display = 'block';
                    break;
            }
        ...
        li.addEventListener('click', e => {
            ...
            if(e.shiftKey){
                ...
                if(firstLi){
                    ...
                    let childNodes = li.parentElement.childNodes
                    childNodes.forEach((element, index) => {
                        if(firstLi === element) startIndex = index;
                        if(li === element) endIndex = index;
                    });
                    let index = [startIndex, endIndex].sort();
                    childNodes.forEach((element, position) => {
